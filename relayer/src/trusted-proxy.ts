 ...
 * configured ourselves. RELAYER_TRUSTED_PROXIES lists the socket addresses
...
export function loadTrustedProxiesFromEnv(): Set<string> {
  return new Set(
    (process.env.RELAYER_TRUSTED_PROXIES ?? "")
      .split(",")
      .map((ip) => ip.trim())
...
export function resolveClientSource(req: IncomingMessage, trustedProxies: Set<string>): string {
